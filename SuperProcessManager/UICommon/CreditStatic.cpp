...
void CCreditStatic::SetCredits(LPCTSTR credits,TCHAR delimiter)
{
	...
	str = new TCHAR[_tcslen(credits)+1];
	ZeroMemory(str,_tcslen(credits)+1);
...
void CCreditStatic::MoveCredit(CDC* pDC, CRect& m_ScrollRect, CRect& m_ClientRect, BOOL bCheck)
{
	...
	if (!m_szWork.IsEmpty()) {
		...
		if(c == m_Escapes[TOP_LEVEL_GROUP]) {
			...
			if (!m_bProcessingBitmap)
			{
				...
				if(m_bmpWork.LoadBitmap((const TCHAR *)szBitmap)) {
					BITMAP 		m_bmpInfo;
	...
	if (!m_bProcessingBitmap)
	{
		...
		int x = memDC.DrawText((const TCHAR *)m_szWork,m_szWork.GetLength()-rmcode,&r,DT_TOP|DT_CENTER|
	...
	while(pos != NULL && height <= m_ScrollRect.Height()) {
		...
		if (!m_szWork.IsEmpty()) {
			...
			if(c == m_Escapes[TOP_LEVEL_GROUP]) {
				...
				CString szBitmap = m_szWork.Left(m_szWork.GetLength()-1);
				if(bitmap.LoadBitmap((const TCHAR *)szBitmap)) {
					BITMAP 		m_bmpInfo;
		...
		{				
			...
			if(m_szWork.GetLength()-rmcode != 0) 
			{
				...
				size = memDC.GetTextExtent((LPCTSTR)TEXT("W"),1);
			height += size.cy;
		}
		else
		{
